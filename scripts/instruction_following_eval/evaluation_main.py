...
def test_instruction_following_strict(
  ...
  response = prompt_to_response[inp.instruction]
  ...
  for index, instruction_id in enumerate(instruction_list):
    ...
    if args and "prompt" in args:
      instruction.build_description(prompt=inp.instruction)
...
def read_prompt_to_response_dict(input_jsonl_filename):
  ...
  return_dict = {}
  if input_jsonl_filename.endswith('jsonl'):
    with open(input_jsonl_filename, "r") as f:
      for l in f:
        example = json.loads(l)
        return_dict[example["prompt"]] = example["response"]
  elif input_jsonl_filename.endswith('json'):
    datas = json.load(open(input_jsonl_filename, "r"))
    for example in datas:
      return_dict[example["instruction"]] = example["output"]
  return return_dict
...
def main(argv):
  ...
  for func, output_file_name in [
      (test_instruction_following_strict, "outputs_eval_results_strict"),
      (test_instruction_following_loose, "outputs_eval_results_loose"),
  ]:
    ...
    output_file_name = os.path.join(
        _INPUT_RESPONSE_DATA.value.replace('/outputs_evaluation.json', ''), output_file_name + ".jsonl"
    )
