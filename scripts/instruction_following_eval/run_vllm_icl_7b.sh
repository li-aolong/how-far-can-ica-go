...
icl_name="base"
icl_path="/home/liyinghao/papers/how_far_ica_go/prompts/if-eval"
...
mkdir -p $output_path
cp /home/liyinghao/papers/how_far_ica_go/scripts/instruction_following_eval/run_vllm_icl_7b.sh $output_path
cp run_vllm.py $output_path
log_file=${output_path}/log.txt

CUDA_VISIBLE_DEVICES=2 output_file_name=$output_file_name batch_size=$batch_size icl_path=$icl_path max_tokens=$max_tokens icl_name=$icl_name dataset=$dataset model_name=$model_name stop=$stop output_path=$output_path model_path=$model_path python run_vllm.py 2>&1 | tee $log_file
