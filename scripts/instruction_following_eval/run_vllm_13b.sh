...
model_path="/home/liyinghao/models/modelscope/Llama-2-13b-ms/modelscope/Llama-2-13b-ms"
icl_name="md_urial_urial"
...
mkdir -p $output_path
cp /home/liyinghao/papers/how_far_ica_go/scripts/instruction_following_eval/run_vllm_13b.sh $output_path
cp run_vllm.py $output_path
log_file=${output_path}/log.txt

CUDA_VISIBLE_DEVICES=0 output_file_name=$output_file_name batch_size=$batch_size icl_path=$icl_path max_tokens=$max_tokens icl_name=$icl_name dataset=$dataset model_name=$model_name stop=$stop output_path=$output_path model_path=$model_path python run_vllm.py 2>&1 | tee -a $log_file
